...
4. Run the bundled generator from the repository root:
   ...
   Replace `HEAD`, `WORKTREE`, and the output path with the requested values. The comparison points can be any Git commit-ish such as `HEAD~1`, a tag, a branch, or a commit SHA. Use `WORKTREE` for the current files. On systems where Python 3 is invoked as `python3`, use that command instead.
